...
function findPlayersWhoWonAHand(rounds: GameRound[]): Set<number> {
...
export function calculateYakitoriPointChanges(
    ...
    if (step === 0) return [];
    const playerIdsWhoWonAHand = findPlayersWhoWonAHand(rounds);
    const yakitori = players.filter(p => !playerIdsWhoWonAHand.has(p.userId));
    const winners = players.filter(p => playerIdsWhoWonAHand.has(p.userId));
