...
read -p "Enter Global Logs Chat ID: " GLOBAL_LOGS_CHAT_ID

if [ -z "$GLOBAL_LOGS_CHAT_ID" ]; then
    echo -e "${RED}Error: Global Logs Chat ID cannot be empty${NC}"
    exit 1
fi
...
BOT_TOKEN=$BOT_TOKEN
GLOBAL_LOGS_CHAT_ID=$GLOBAL_LOGS_CHAT_ID
EOF"
