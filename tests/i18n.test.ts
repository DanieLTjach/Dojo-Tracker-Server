...
describe('i18n core', () => {
    ...
    it('interpolates {{param}} placeholders', () => {
        expect(t('errors.gameNotFoundById', DEFAULT_LOCALE, { id: 42 })).toBe('Гру з id 42 не знайдено');
    });

    it('returns the template verbatim when no params object is passed', () => {
        expect(t('errors.gameNotFoundById', DEFAULT_LOCALE)).toBe('Гру з id {{id}} не знайдено');
    });
...
describe('error catalog coverage', () => {
    ...
    const allErrors: ResponseStatusError[] = [
        ...
        new PointCalculationErrors.HandDetailRequiredError(),
        new PointCalculationErrors.HandDetailNotSupportedForSanmaError(),
        new PointCalculationErrors.InvalidHandDetailStructureError(),
        new PointCalculationErrors.NonWinningHandError(),
        new PointCalculationErrors.HandHasNoYakuError(),
