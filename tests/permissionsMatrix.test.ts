...
describe('Permissions matrix integration specification', () => {
    ...
    function insertGameRules(
        ...
        dbManager.db.prepare(
            `INSERT INTO gameRules (id, name, numberOfPlayers, uma, startingPoints, clubId)
             VALUES (?, ?, ?, ?, ?, ?)`
        ).run(gameRulesId, name, numberOfPlayers, '[15,0,-15]', startingPoints, clubId);
    }
    ...
    function insertGame(gameId: number, eventId: number): void {
        const timestamp = nextTimestamp();
        dbManager.db.prepare(
            `INSERT INTO game (id, eventId, createdAt, modifiedAt, modifiedBy, tournamentRound, tournamentTable, status, startedAt, endedAt)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
