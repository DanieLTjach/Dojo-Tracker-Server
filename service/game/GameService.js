...
export class GameService {
    ...
    async addGame(type, players_data, modified_by, created_at, club_id, event_id) {
        try {
            ...
            const result = await this.db.add_game(type, players_data, modified_by, created_at, club_id, event_id);
    ...
    async removeGame(game_id, modified_by) {
        try {
            const result = await this.db.remove_game(game_id, modified_by);
            if (result.success === true) {
    ...
    async listGames(type, date_from, date_to, user_id, club_id) {
        try {
            const result = await this.db.list_games(type, date_from, date_to, user_id, club_id);
            if (result.success === true) {
    ...
    async getGame(game_id) {
        try {
            const result = await this.db.get_game(game_id);
            if (result.success === true) {
                return { success: true, result: result.result };
            }
